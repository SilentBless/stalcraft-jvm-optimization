...
## Помощь проекту — опциональный сбор данных

Если хочешь помочь — прогони стандартизированный CapFrameX-замер и прикрепи его к [Discussions](../../discussions). Запись **не содержит личных данных** (никаких путей, ID, IP) — только модель железа и фреймтайм-сэмплы.

Данные сообщества помогают валидировать JVM-тюнинг на железе, до которого у нас нет доступа. Протокол теста описан в [BENCHMARKS.md](./docs/BENCHMARKS.md) — условия единые для всех, иначе результаты сравнивать нельзя.
