...
> [!NOTE]
> По умолчанию доступна только конфигурация `default.json`, однако она *не является* единственной опцией.
> В разделах [Примеры конфигурации](#примеры-конфигурации) и [Кастомная конфигурация](#кастомная-конфигурация)
> представлены необходимые инструкции.

#### Примеры конфигурации

На данный момент в репозитории лежит один пример — `examples/8khz.json`, ориентированный на high-end системы (8+ ядер, 32 ГБ ОЗУ) с мышью 8 kHz. Он делает упор на минимальные STW-паузы и предсказуемый фреймтайм в ущерб небольшому throughput'у.
...
Оба бинарника пишут структурированные логи в каталог `jvm_wrapper/logs/` рядом с `cli.exe` и `service.exe`:

- **`logs/wrapper.log`** — события утилиты: запуск, детект железа, загрузка конфига, старт игрового процесса, код выхода. Путь пользователя редактируется до `<user>`, сырые аргументы лаунчера и JVM-флаги в лог не пишутся. Файл усекается при превышении 2 МБ.
- **`logs/jvm.log`** — [unified logging JVM](https://openjdk.org/jeps/158): GC-паузы, все STW safepoint'ы, JIT-компиляции, deoptimization, metaspace, code cache. 3 файла по 10 МБ с ротацией, суммарно до 30 МБ на диске. Позволяет диагностировать причины микрофризов.

Если вы столкнулись с проблемой и хотите сообщить о ней — приложите оба файла из `jvm_wrapper/logs/` к issue на GitHub. Они не содержат личной информации и безопасны для публикации.
